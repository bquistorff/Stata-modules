...
//normal assert doesn't allow a message
void assert_msg(real scalar logical, string scalar message){
	if(!logical){
		_error(message)
	}
}

real colvector per_row_weighted_cross(real matrix X, real matrix W){
	N=rows(X)
	K=cols(X)
	ret = J(N,1,0)
