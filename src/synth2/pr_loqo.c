...
{
	SF_error_const("ERROR: terminating optimizer - ");
	SF_error_const(error_text);
	SF_error_const("\n");
}
...
/* BQ: The "leaves UR triangle intact implies that i indexes columns
and therefore uses the Cholesky–Crout algorithm and matrix is col major,
but this seems to disagree with "rows first order" which appears to 
imply row-major ordering. */
int choldc(double a[], int n, double p[])
{
	...
	for (i = 0; i < n; i++){
		/*j=i*/
		sum=a[n*i + i];
		for (k=i-1; k>=0; k--) sum -= a[n*i + k]*a[n*i + k];
		if (sum <= 0.0) {
			sum = 0.0;
			ret++;
		}
	...
	double workspace[], int step)
{
	...
	if (step == PREDICTOR) {
		int ch_ret = choldc(h_x, n, p_x);	/* do cholesky decomposition */
	...
	double margin, double bound, int restart) 
{
	...
	/* the main loop */
	if (verb >= STATUS) {
		SF_display_const("counter | pri_inf  | dual_inf  | pri_obj   | dual_obj  | ");
		SF_display_const("sigfig | alpha  | mu \n");
		SF_display_const("-------------------------------------------------------");
		SF_display_const("---------------------------\n");
	}
	int sr_ret = 0;
