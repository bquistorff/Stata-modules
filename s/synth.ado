...
program synth , eclass
	...
	*BQ Optim: Make the donor indicator once!
	tempvar counit_ind xtime_ind mean_var
	qui gen byte `xtime_ind' = 0
	...
	if `checkinput' == 0 {
		di as err "period of treatment is not found in timevar - check trperiod()"
		exit 198
	}
	...
	while "`anything'" != "" {
		...
		/* Controls *************************** */
		if `num_xtime'==1 { //BQ optimization
			...
			qui reducesample , tno("`xtime'") uno("`counit'") genname(`subsample') tvar(`tvar') pvar(`pvar')

			/* Deep Missing Checker (may be omitted to gain speed) */
			missingchecker , tno("`xtime'") cvar("`var'") sub("`subsample'") ulabel("control units") checkno(`cono') tilab("`xtimelab'") tvar(`tvar')

			/* aggregate over years */
			agmat `Xcotemp' , cvar(`var') sub(`subsample') ulabel("control units") checkno(`cono') tilab("`xtimelab'") pvar(`pvar')
			qui drop `subsample'
			...
			/* Deep Missing Checker (may be omitted to gain speed)  */
			missingchecker , tno("`xtime'") cvar("`var'") sub("`subsample'") ulabel("treated unit") checkno(`trno') tilab("`xtimelab'") tvar(`tvar')
			...
			/* BQ Opt: Untested
			tempname Xcotemp2 Xtrtemp2 Xcotemp3 Xtrtemp3
			local tx: subinstr local xtime " " ",", all
			qui replace `xtime_ind' = inlist(`tvar',`tx')
			by `pvar': egen `mean_var' = mean(`var') if `xtime_ind'
			*Now just pick out the first (could use any)
			local xtime1 : word 1 of `xtime'
			mkmat `mean_var' if `counit_ind'     & `tvar'==`xtime1', matrix(`Xcotemp2') rownames(`pvar')
			mkmat `mean_var' if `pvar'==`trunit' & `tvar'==`xtime1', matrix(`Xtrtemp2') rownames(`pvar')
			drop `mean_var'
			
			mat `Xcotemp3' = (`Xcotemp',`Xcotemp2')
			mat `Xtrtemp3' = (`Xtrtemp',`Xtrtemp2')
			set trace off
			mat li `Xcotemp3'
			mat li `Xtrtemp3'
			pause */
	...
	tempname Xbal Zbal Ybal loss Xsynth Ysynth Zsynth gap H c A b l u wsol wsol_unr Xtr_norm Xco_norm
	...
	mat drop xcomat xtrmat vmat emat fmat matout
...
program reducesample , rclass
	version 9.2
	syntax , tno(numlist >=0 integer) uno(numlist integer) genname(string) tvar(string) pvar(string) [u_ind(string)]
...
program missingchecker , rclass
  version 9.2
  syntax , tno(numlist >=0 integer) cvar(string) sub(string) ulabel(string) checkno(string) tvar(string) [ tilab(string) ]
