...
{p2colset 5 20 22 2}{...}
{p2col :{hi:synth2} {hline 2}}Synthetic control methods for comparative case studies  {p_end}
{p2colreset}{...}
...
{p 6 8 2}
{opt synth2} {help synth2##predoptions:{it:depvar}}  {help synth2##predoptions:{it:predictorvars}} , {opt tru:nit}({it:#}) {opt trp:eriod}({it:#}) [ {opt cou:nit}({it:{help numlist:numlist}})  {cmd: xperiod}({it:{help numlist:numlist}}) 
{cmd: mspeperiod}() {cmd: resultsperiod}() {cmd: nested} {cmd: allopt} {cmd: unitnames}({it:{varname}}) {opt fig:ure}  {opt k:eep}({it:file}) {cmd: customV}({it:{help numlist:numlist}}) {help synth2##osettings:{it:optsettings}} ]
...
{p 4 4 2}
{cmd:synth2} implements the synthetic control method for causal inference in comparative case studies. {cmd:synth2}
...
to the
evolution of the same aggregate outcome for a synthetic control group. {cmd:synth2}
...
affected unit in the absence of the intervention. {cmd:synth2} can also be used to 
...
variable will be averaged. For this, {cmd: synth2} uses a specialized syntax.
...
{p 10 10 2}
{cmd:. synth2 Y X1(1980) X2(1982&1986&1988) X3(1980(1)1990) X4}{p_end}
...
{cmd: xperiod}({it:numlist}) a list of time periods over which the predictor variables
 specified in {help synth2##predoptions:{it:predictorvars}}
...
{p 4 8 2}
{cmd: resultsperiod}({it:numlist}) a list of time periods over which the results of {cmd: synth2} 
...
{p 4 8 2}
{cmd:nested} by default {cmd: synth2} uses a data-driven regression based
...
{cmd:synth2} embarks on an fully nested optimization procedure
...
his application (see  {help synth2##info:{it:optimset}}).
...
{cmd: synth2} returns the best result of all three attempts. This option
...
{p 4 8 2}
{cmd: figure} if specified {cmd:synth2} produces a line plot with outcome trends for the treated unit and the synthetic control unit 
...
{p 4 8 2}
{cmd:customV}({it:numlist}) by default {cmd: synth2} uses a data-driven regression based
...
listed in  {help synth2##predoptions:{it:predictorvars}}.
...
constrained quadratic programming routine and Stata's {help ml} optimizer. By default, {cmd: synth2}
...
synth2 ... , iterate(20) ).{p_end}
{synoptline}

{title:Saved Results}

{p 4 8 2}
By default, {cmd:synth2} ereturns the following matrices, which 
can be displayed by typing {cmd: ereturn list} after 
{cmd: synth2} is finished (also see {help ereturn}).  
...
{phang}{stata synth2 cigsale beer(1984(1)1988) lnincome retprice age15to24 cigsale(1988) cigsale(1980) cigsale(1975), trunit(3) trperiod(1989)}
...
{phang}{stata synth2 cigsale beer  lnincome(1980&1985)  retprice  cigsale(1988) cigsale(1980) cigsale(1975), trunit(3) trperiod(1989) fig}
...
Since no data is available for beer prior to 1984, {cmd: synth2} will inform the user 
...
{phang}{stata synth2 cigsale retprice cigsale(1970) cigsale(1979) , trunit(33) counit(1(1)20) trperiod(1980) fig resultsperiod(1970(1)1990)}
...
{phang}{stata synth2 cigsale retprice cigsale(1970) cigsale(1979) , trunit(33) counit(1(1)20) trperiod(1980) resultsperiod(1970(1)1990) keep(resout)}

{p 8 8 2}
This example is similar to example 2 but {cmd: keep(resout)} is specified and thus {cmd: synth2} will save a dataset named resout.dta
in the current Stata working directory (type {cmd: pwd} to see the path of your working directory). This dataset contains the 
result from the current fit and can be used for further processing. Also to easily access results recall that {cmd: synth2} routinely returns all result matrices. These can be displayed by typing {cmd: ereturn list} after 
{cmd:synth2} has terminated.{p_end}

{p 4 8 2}
Example 5 - Construct synthetic control group:{p_end}
{phang}{stata synth2 cigsale beer lnincome retprice age15to24 cigsale(1988) cigsale(1980) cigsale(1975) , trunit(3) trperiod(1989) xperiod(1980(1)1988) nested}
...
{cmd:      synth2 cigsale retprice cigsale(1988) cigsale(1980) cigsale(1975) , trunit(`i') trperiod(1989) xperiod(1980(1)1988) } {break}
